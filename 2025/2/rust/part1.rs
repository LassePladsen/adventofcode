...
/// To decide what length the substring we should check, we iterate through the digits until we hit
/// a new one.
fn valid(id: u128) -> bool {
    ...
    // Now that we have the substring length we wish to use, parse through the string with this
    // length and check for duplicate counts
